...
public class OutsourcingTestController {
    ...
    public Result<Page<OutsourcingTest>> getTests(
    ...
    public Result<OutsourcingTest> createTest(@Valid @RequestBody OutsourcingTest test) {
        OutsourcingTest createdTest = outsourcingTestService.createTest(test);
        return Result.success(createdTest);
    }
    ...
    public Result<List<OutsourcingTest>> getMyTests() {
        List<OutsourcingTest> tests = outsourcingTestService.getTestsForCurrentUser();
        return Result.success(tests);
    }
    
    @Operation(summary = "获取测试统计", description = "获取测试任务的统计数据")
    ...
    public Result<?> getTestStatistics(
        ...
        Object statistics = outsourcingTestService.getTestStatistics(startDate, endDate);
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('PROJECT_MANAGER')")
    public Result<?> getTesterPerformance(
            @Parameter(description = "测试人员ID") @RequestParam(required = false) Long testerId,
            @Parameter(description = "统计月份") @RequestParam(required = false) String month) {
        Object performance = outsourcingTestService.getTesterPerformance(testerId, month);
